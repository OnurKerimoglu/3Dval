...
def main(dataroot, yearlist, scenarios, lonlatextent, varns, mos):
    ...
    scenout = {'CS':'CS','28':'2.8m','28M':'2.8o'}
    # Color limits:
    vlimsNull = [0, 0]
    if not mos == '_std':
    ...
    if len(scenarios) == 2:
        ...
        comparescenarios = [len(scenarios) - 2, len(scenarios)-1]
    
    # string together all scenarios for output name
    ...
    for y in yearlist:
        ...
        for si, scen in enumerate(scenarios):
            ...
            for vari, varn in enumerate(varns):
                ...
                # define title string
                titlestr = prettytitle[varn] + mos2 + ' ' + scenout[scen] + ' [$' + units[varn] + '$]'
        ...
        if len(scenarios) > 1:
            for vari, varn in enumerate(varns):
                ...
                tmpdiff = amshp[f'{varn}_{scenarios[comparescenarios[1]]}'] - amshp[
                    f'{varn}_{scenarios[comparescenarios[0]]}']
                columndiff = f'{varn}{mos}_diff'
                amshp[columndiff] = tmpdiff

                # define title string
                titlestr = prettytitle[
                               varn] + f'{mos2} difference {scenout[scenarios[comparescenarios[1]]]}-{scenout[scenarios[comparescenarios[0]]]} [$' + \
                           units[varn] + '$]'
        ...
        if print_plot_switch:
            if mos == '_std':
                outnamestd=f'{dataroot}WFD_area_std_{scenariostring}.jpg'
                plt.savefig(outnamestd, dpi=dpi)
                print(f'{outnamestd} saved!')
            else:
                outnamemean=f'{dataroot}WFD_area_means_{scenariostring}.jpg'
                plt.savefig(outnamemean, dpi=dpi)
                print(f'{outnamemean} saved!')
        if show_plot_switch:
            plt.show()
...
def plot_panel(f, nrows, panelindex, amshp, column, vlims, lonlatextent, titlestr):
    ...
    if vlims[0] == vlims[1]:
        ...
        #amshp.plot(column=column, legend=True, ax=ax, scheme = "User_Defined",
                   #classification_kwds = dict(bins=[0,4,6,8,10,12,14,16,18,20]), cmap="Greens")
