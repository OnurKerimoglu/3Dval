...
def define_varlims(scen1,scen2,var):
    if 'HS' not in scen1 and 'HS' not in scen2:
        ...
        lim_vars = {'Chl': 25, 'DIN': 50, 'DIP': 50}
        cb_interval_dict = {'Chl': 5, 'DIN': 5, 'DIP': 5}
        cb_interval = int(cb_interval_dict[var])
        cbticks_dict = {'Chl': 5, 'DIN': 10, 'DIP': 10}
        ...
        lim_vars = {'Chl': 50, 'DIN': 10, 'DIP': 80}
        cb_interval_dict = {'Chl': 10, 'DIN': 1, 'DIP': 10}
        cb_interval = int(cb_interval_dict[var])
        cbticks_dict = {'Chl': 10, 'DIN': 2, 'DIP': 10}
        cbticks = cbticks_dict[var]
...
def main(scen1, scen2, varns, plfpath):
    ...
    for e, coords in ext.items():
        for vari, varn in enumerate(varns):
            ...
            titlestr = f"{prettytitle[varn]} 100*({scen2out}-{scen1out})/{scen1out}"
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 4:
        ...
        plfpath = '/home/daniel/levante_work/IR/Harmonization/diffmaps'
